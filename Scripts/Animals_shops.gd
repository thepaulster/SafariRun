...
onready var animals_panel = get_node("BackgroundView/Pagebg/ScrollContainer/VBoxContainer")
...
func _ready():
	...
	for item in range (animals_panel.get_child_count()-1):
		if Globals.store.bought[item] == true:
			animals_panel.get_node("Player_Card_" + str(item + 1)).get_node("HBoxContainer/VBoxContainer2/TextureButton/Label").text = 'Select'
	animals_panel.get_node("Player_Card_" + str(Globals.store.selected + 1)).get_node("HBoxContainer/VBoxContainer2/TextureButton/Label").text = ' selected '
	animals_panel.get_node("Player_Card_" + str(Globals.store.selected + 1)).get_node("HBoxContainer/VBoxContainer2/TextureButton/Label").add_to_group('selected')
